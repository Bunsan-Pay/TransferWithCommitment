...
contract SignatureTransferTest is Test {
    ...
    /// @notice `transferWithAuthorization` (CommittedTransferDetail[]) — batch with per-commitment guards
    function test_transferWithAuthorization_committedBatch() public {
        ...
        bytes32 structHash = keccak256(
            abi.encode(twc.BATCH_TRANSFER_WITH_COMMIT_TYPEHASH(), alice, executor, batchHash, validAfter, validBefore)
        );
        bytes32 digest = twc.hashTypedDataV4(structHash);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alicePk, digest);
        vm.expectEmit(true, true, true, true);
        ...
        twc.transferWithAuthorization(alice, details, validAfter, validBefore, abi.encodePacked(r, s, v));
    }
    ...
    function test_transferWithAuthorization_revertsEmptyCommittedDetails() public {
        ...
        twc.transferWithAuthorization(alice, details, 0, type(uint256).max, hex"");
    }
