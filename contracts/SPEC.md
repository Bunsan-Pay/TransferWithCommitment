...
| `transferWithAuthorization(from, details, validAfter, validBefore, batchCommitment, signature)`（`CommittedTransferDetail[]`） | `nonReentrant`；`validTimestamp`；`replayGuard(from, batchCommitment)`；`require(details.length > 0)`（空配列は revert）；`digest` に対し `SignatureChecker.isValidSignatureNow(from, digest, signature)` が真（`details` は `Hash.hashBatchCommittedTransferDetails(details)` と同手順で折り畳み、各 `detail.commitment` を含む；struct に `batchCommitment` を含む） | 成功時、まず `(from, batchCommitment)` が使用済みとなり、続けて `_batchTransfers` が各明細ごとに `_transferWithGuard` を呼び、**各** `(from, detail.commitment)` も使用済みとなり、イベントには各 `detail.commitment` が載る | `Hash.hashBatchCommittedTransferDetails(details)` → structHash（`batchCommitment` 含む）→ 検証 → `_batchTransfers` |
...
- コミットメントの唯一性（再利用防止）
  ...
  `CommittedTransferDetail[]` 経路では、署名付きバッチでは EIP-712 の `batchCommitment` に対して `replayGuard(from, batchCommitment)` が先に実行され、続いて各明細について `_transferWithGuard` 経由で `(from, detail.commitment)` が検証される。Self-Call の `transfer(CommittedTransferDetail[])` には `batchCommitment` は無く、明細ごとの `replayGuard` のみ。
...
- ERC-1271 経路の呼び出し順
  ...
  委任経路（`SignatureTransfer`）では **`replayGuard` → `SignatureChecker.isValidSignatureNow` → `_transfer`** の順である。`replayGuard` は修飾子として関数本体より先に走り、その中で `commitmentUsed` により当該 `(from, commitment)` を使用済みにマークしてから本体に入るため、**コントラクト署名者に対する `isValidSignatureNow` 内部の ERC-1271 `isValidSignature` は、コミットメント消費のあとに呼ばれる**。

  この順序のため、ウォレットの `isValidSignature` 実装が同一トランザクション内で `commitmentState(from, commitment)` を読むと **使用済み（`true`）** を観測する。多くの用途では無害であり、オンチェーン状態に依存せず署名検証のみ行う ERC-1271 ウォレットでは実質問題にならない。
...
- 明細配列 `details` の長さ（オンチェーン上限なし）

  `SelfTransfer` および `SignatureTransfer` のバッチ／ユニファイド経路では、`details` の要素数にコントラクト側の上限はない。ループ処理の途中で allowance 不足などにより revert した場合、それまでに消費したガスに加え、失敗時の revert コストも含め、**トランザクションを負担する `msg.sender`（Self-Call）または署名経路の executor（トランザクション送信者）**が高コストになりうる。配列長の見積り・上限の設定、送信前のシミュレーション、署名ペイロードの妥当性確認などは、**本スタックを利用するアプリケーションの責務**とし、本リポジトリのコントラクト実装では緩和しない（SDK 等による事前シミュレーションやオフチェーン検証で抑止することを想定する）。
